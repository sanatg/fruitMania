...
function draw(){
 ...
 spawnobstacles();
 runner.scale = 1.5;
 startPoint.position.x = startPoint.position.x-2;
 starterText.position.x = starterText.position.x-2;
 if(keyIsDown(UP_ARROW) && (gameState)){
 runner.position.y = runner.position.y-4
...
if(runner.collide(obstaclesGroup)){
runner.addImage(runnerstarterImg);
gameState = end;
